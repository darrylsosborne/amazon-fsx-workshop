...
Below is a table and charts with the results from some previous tests. These results show a significant improvement when an Amazon EC2 Windows instance (us-west-2) reads this large test file from the local FSx File Gateway (us-west-2) compared to a remote FSx for Windows File Server (us-east-1).

image::read-results-table.png[title="DiskSpd Read Results Table - sample", align="left", width=600]

image::read-results-throughput-chart.png[align="left", width=600]
* DiskSpd Read Throughput Chart - sample

image::read-results-latencies-chart.png[align="left", width=600]
* DiskSpd Read Latencies Chart - sample


[width="50%",cols="d,d,d,d,d",frame="topbot",options="header"]
|===
h| EC2 Instance (us-west-2) 2+h| FSx for Windows File Server (us-east-1) 2+h| FSx File Gateway (us-west-2)
s| DiskSpd Read (metric) h| Test 1 (ms) h| Test 2 (ms) h| Test 1 (ms) h| Test 2 (ms)
| Read IO throughput (MiB/s)
>m| 189.71
>m| 166.07
>m| 355.43

...
The following table and charts show the results from a previous test. These results show a significant improvement when an Amazon EC2 Windows instance (us-west-2) reads a large file from the local FSx File Gateway (us-west-2) compared to a remote FSx for Windows File Server (us-east-1). Remember, the initial read from the FSx File Gateway will experience higher latencies as the FSx File Gateway reads the file into its cache from the remote FSx for Windows File Server. Subsequent reads deliver the file directly from the local FSx File Gateway.

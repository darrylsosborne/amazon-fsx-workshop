...
. Review the PowerShell function commands available using the *Amazon FSx CLI for remote management on PowerShell*. *_Run_* *Get-Command*.

. What commands are available?

. Examine your file system using the commands in the table below. *_Run_* the commands in the remote PowerShell session.
+
|===
| *Command*
| Get-FSxSmbShare

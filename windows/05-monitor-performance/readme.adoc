...
* Did your email address receive an alarm notificaiton?
* What triggered the alarm?
. From the link:https://console.aws.amazon.com/cloudwatch/[Amazon CloudWatch] console, *_select_* *Dashboards* from the left navigation pane.
...
```sh
$random = $(Get-Random)
C:\Tools\fio-3.13-x64\fio --directory Z\:\\ --randrepeat=1 --direct=1 --name=${env:computername}-$random --numjobs=1 --bs=64K --iodepth=32 --size=1024M --readwrite=read --rwmixread=100 --thread --time_based --runtime=120
```
+
While the script is running, open *Task Explorer* and monitor network performance (e.g. Task Explorer >> Performance (tab) >> Ethernet).
+
...
```sh
$random = $(Get-Random)
C:\Tools\fio-3.13-x64\fio --directory Z\:\\ --randrepeat=1 --direct=1 --name=${env:computername}-$random --numjobs=1 --bs=64K --iodepth=32 --size=1024M --readwrite=read --rwmixread=0 --thread --time_based --runtime=120
```
+
While the script is running, open *Task Explorer* and monitor network performance (e.g. Task Explorer >> Performance (tab) >> Ethernet).
+
